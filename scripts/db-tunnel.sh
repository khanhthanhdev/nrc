#!/bin/bash
# Script để setup SSH tunnel cho database (Production hoặc Staging)
# Sử dụng: ./scripts/db-tunnel.sh [start|stop|status] [--host SSH_HOST] [--key SSH_KEY]
#
# Lưu ý: Script này hữu ích khi cần kết nối trực tiếp đến database trên server từ xa
...
case "$ACTION" in
    start)
        echo "=== STARTING DATABASE SSH TUNNEL ==="
        echo ""
        echo "📋 Configuration:"
        ...
        if [ $SSH_EXIT_CODE -eq 0 ] && check_port_listening $LOCAL_PORT; then
            echo "✅ Database tunnel started successfully!"
            echo ""
            echo "📋 Connection:"
            echo "   Local:  postgresql://localhost:$LOCAL_PORT"
            echo "   Remote: $SSH_HOST:$REMOTE_PORT"
            echo ""
            echo "💡 To stop tunnel: ./scripts/db-tunnel.sh stop"
            echo ""
            echo "📝 DrizzleKit usage:"
            echo "   DATABASE_URL=\"postgresql://localhost:$LOCAL_PORT\" bunx drizzle-kit introspect"
        else
            echo "❌ Failed to start database tunnel!"
            echo "   Please check:"
            echo "   1. SSH key file exists: $SSH_KEY"
            echo "   2. SSH connection works: ssh -i \"$SSH_KEY\" $SSH_HOST"
            echo "   3. Port $LOCAL_PORT is not already in use"
            exit 1
        fi
        ;;

    stop)
        echo "=== STOPPING DATABASE SSH TUNNEL ==="
        echo ""
    ...
    status)
        echo "=== DATABASE SSH TUNNEL STATUS ==="
        echo ""

        PID=$(get_pid_from_port $LOCAL_PORT)
        if [ -n "$PID" ]; then
            echo "✅ Database tunnel: RUNNING"
            echo "   PID: $PID"
            echo "   Port: $LOCAL_PORT"
            echo "   Remote: $SSH_HOST:$REMOTE_PORT"
            echo ""
            echo "📋 Test connection:"
            echo "   psql postgresql://localhost:$LOCAL_PORT"
            echo ""
            echo "📝 DrizzleKit:"
            echo "   DATABASE_URL=\"postgresql://localhost:$LOCAL_PORT\" bunx drizzle-kit introspect"
        else
            ...
            echo "   Port: $LOCAL_PORT"
            echo ""
            echo "💡 To start: ./scripts/db-tunnel.sh start"
        fi
        ;;
    ...
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  start  - Start SSH tunnel for database"
        echo "  stop   - Stop SSH tunnel"
        echo "  status - Check tunnel status"
        echo ""
        echo "Options:"
        echo "  --host SSH_HOST      SSH host (default: ec2-user@54.255.37.156)"
        echo "  --key SSH_KEY        SSH key file (default: ~/.ssh/steam4VN-key.pem)"
        echo "  --local-port PORT    Local port (default: 5432)"
        echo "  --remote-port PORT   Remote port (default: 5432)"
        echo ""
        echo "Examples:"
        echo "  $0 start"
